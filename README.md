...
## EfficientNetB0 Flood Classification

ResQAI uses **EfficientNetB0** as the core vision intelligence model for flood detection and disaster severity analysis.

### Why EfficientNetB0?

Disaster-response systems require:
- fast inference
...
Unlike heavier architectures such as ResNet50 or Vision Transformers, EfficientNetB0 achieves high performance with significantly fewer parameters through compound scaling of:
- depth
- width
- resolution
...
- edge deployment
- real-time humanitarian operations

---

## Vision Pipeline Capabilities

The Vision Intelligence Agent performs:
- flood detection
- disaster severity estimation
- environmental hazard recognition
- operational flood analysis

Outputs include:
- flood confidence scores
...
To improve transparency during emergency response, ResQAI integrates **Grad-CAM explainability**.

Grad-CAM visualizes the exact image regions influencing flood predictions, allowing responders to:
- audit AI reasoning
- identify high-risk flood regions
- improve operational trust
- reduce black-box decision making

This is especially important in humanitarian environments where explainability and trust are critical.

---

## Operational Importance in ResQAI

EfficientNetB0 acts as the visual intelligence component inside the larger multimodal fusion system.

Flood intelligence generated by the vision pipeline is fused with:
- multilingual distress speech
- weather escalation signals
- grounded humanitarian retrieval
- temporal crisis memory

through the **Gemma Fusion Layer** to generate coordinated operational disaster intelligence.
